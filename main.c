...
int main()
{
	if ((fptr = fopen("report.txt", "w")) == NULL)
...
void findBestPlay(struct player playersArray[])
{
	...
	if (foldCount != playerNum - 1) {
		...
		if (count > 1)
		{
			...
			there is a tie between high Cards, in getHighCards record what high card was
			but invalidate the value in deltCards for if another tie occurs*/

			/* 2d array done using double pointers, deltCards holds array of pointers to an array of pointers that point to card structures
			first allocate memory for rows, playerNum rows that hold struct card pointers,
			second allocate memory for the columns, 2 card structs.*/
